...
export function Shortener() {
  ...
  const inputError = error
    ? "text-red-brand outline-2 outline-red-brand"
      ...
      <div className="px-6 bg-[linear-gradient(to_bottom,_white_0%,_white_50%,_#f0f1f6_50%,_#f0f1f6_100%)] mt-15">
        ...
        >
          <div className="flex flex-col gap-2 w-full md:relative">
            ...
            ></input>
            {error && (
              <p className="text-red-400 text-sm italic md:absolute md:-bottom-7">
                {error}
              </p>
            )}
          </div>
...
function UrlDiv({ data, handleDelete }) {
  ...
  const bgColor = copied
    ? "bg-violet-brand"
    : "bg-cyan-brand hover:bg-cyan-hover";
  ...
  return (
    <div className="flex flex-col md:flex-row md:items-center gap-3 md:gap-5 rounded-lg bg-white pb-4 md:px-5 md:py-2 max-w-5xl w-full mx-auto shadow-xs">
      <p className="text-very-dark-violet px-4 md:px-0 py-3 md:py-0 border-b-1 md:border-b-0 border-b-neutral-200 md:w-full text-ellipsis overflow-scroll whitespace-nowrap">
        {data.fullUrl}
      </p>
      <a
        href={`https://${data.shortUrl}`}
        target="_blank"
        className="text-cyan-brand px-4 md:px-0 md:min-w-fit hover:underline"
      >
        {data.shortUrl}
      </a>
...
function LinksList({ data, handleDelete }) {
  ...
  return (
    <section className="flex flex-col gap-3 p-6 bg-gray-section">
      {data.map((url) => (
        <UrlDiv key={url.shortUrl} data={url} handleDelete={handleDelete} />
      ))}
    </section>
  );
}
